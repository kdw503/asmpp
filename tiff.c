                                   ...
                                   struct DataLocation *DataLoc )
{
  ...
  if ( DataLoc->rows_per_strip > ( (unsigned int) img->height ) )
    DataLoc->rows_per_strip = (unsigned int) img->height;
                                  ...
                                  struct DataLocation *DataLoc )
{
  ...
  if ( ( img->TIFF_type == 'g' ) || ( img->TIFF_type == 'p' ) ) {
    DataLoc->bytes_per_row = ( unsigned int ) ( img->width );
  }
  else if ( img->TIFF_type == 'c' ) {
    DataLoc->bytes_per_row = ( unsigned int ) ( 3 * img->width );
  }
  else {
...
{
  struct TIFF_field *field;
  unsigned int max_bytes_in_one_strip, i;
                                         ...
                                         int dimension )
{
  ...
  if ( dimension < 65536 ) {
    ...
    field->Type = LONG;
    field->Value.ULong = ( unsigned int ) dimension;
  }
...
static int MakeDefaultRowsPerStripField ( struct IFD *ifd )
{
  ...
  if ( height < 65536 ) {
    ...
    field->Type = LONG;
    field->Value.ULong = ( unsigned int ) height;
  }
...
static int UnpackStrip ( struct TIFF_img *img,
                         ...
                         unsigned char *buffer )
{
  unsigned int i, first_row, bytes_unpacked, current_row;
  int j;
                                     ...
                                     unsigned int num_elements )
{
  ...
  if ( field->Type == SHORT ) {
    if ( field->Count == 1 )
      (*array)[0] = ( unsigned int ) field->Value.UShort;
    else {
      for ( i = 0; i < num_elements; i++ )
        (*array)[i] = ( unsigned int ) ( field->Value.UShortArray[i] );
    }
  }
  else if ( field->Type == LONG ) {
                             ...
                             unsigned int *rows_per_strip )
{
  ...
  if ( field->Type == SHORT )
    *rows_per_strip = ( unsigned int ) field->Value.UShort;
...
static int AllocateColorMap ( unsigned char ***cmap, struct IFD *ifd )
{
  ...
  cmap_length = ( unsigned int ) pow ( 2.0, ( double ) bits_per_sample );

  /* ensure the ColorMap field has same number for its count */
  if ( ( field = GetFieldStructure ( ifd, ColorMap ) ) == NULL )
    return ( ERROR );
  if ( field->Count != ( unsigned int ) ( 3 * cmap_length ) ) {
                                                  ...
                                                  unsigned short i )
{
  ...
  PositionValue = OffsetOfIFD + 2 + ( (unsigned int) (i) * 12 );
...
static int AllocateArrayOfValues ( struct TIFF_field *field )
{
  ...
  if ( field->Type == LONG )
    if ( ( field->Value.ULongArray =
           ( unsigned int * )
           calloc ( ( size_t ) field->Count, sizeof ( unsigned int ) ) )
         == NULL )
      return ( ERROR );
...
static int GetUnsignedLong ( FILE *fp, unsigned int *UnsignedLong )
{
  ...
  if ( 1 != fscanf ( fp, "%4c", (char *)(&unsignedlong) ) ) {
    fprintf ( stderr,"error reading unsigned int\n" );
    return ( ERROR );
  }
